...
export default function FolderDetailScreen({ route, navigation }) {
  ...
  let currentFolders = folders.filter(f => f.parentId === folderId);
  if (sortMode === 'alpha_asc') currentFolders.sort((a, b) => a.name.localeCompare(b.name));
  else if (sortMode === 'items_desc') currentFolders.sort((a, b) => getFolderItemCount(b.id) - getFolderItemCount(a.id));
  else currentFolders.sort((a, b) => b.id.localeCompare(a.id));

  const currentVideos = videos.filter(v => v.folder_id === folderId);
  const matchingVideos = searchQuery.trim()
    ? videos.filter(v => v.folder_id === folderId && v.title.toLowerCase().includes(searchQuery.toLowerCase()))
    : currentVideos;
  ...
  const handleSaveFolder = () => {
    if (newFolderName.trim()) {
      if (editingFolder) {
        ...
        addFolder(newFolderName.trim(), selectedColor, 'folder', folderId);
      }
      setNewFolderName('');
      setEditingFolder(null);
    ...
    <View style={[styles.container, { backgroundColor: bg }]}>
      ...
      <View style={[styles.headerContainer, { borderBottomColor: border }]}>
        ...
        <View style={styles.headerTextContainer}>
          <Text style={[styles.headerTitle, { color: text }]} numberOfLines={1}>{folderName}</Text>
          <Text style={[styles.headerSub, { color: subText }]}>
            {currentFolders.length} folders · {currentVideos.length} videos
          </Text>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {currentFolders.length > 0 && (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: text }]}>Folders</Text>
            <View style={styles.gridContainer}>
              {currentFolders.map((folder) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.75}
                  onPress={() => navigation.push('FolderDetail', { folderId: folder.id, folderName: folder.name })}
                >
                  <View style={styles.folderCardHeader}>
                    ...
                    <TouchableOpacity
                      onPress={() => handleOptionsPressFolder(folder)}
        ...
        {matchingVideos.length > 0 && (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: text }]}>
              {searchQuery.trim() ? 'Results' : 'Videos'}
            </Text>
            {matchingVideos.map((video) => (
              <TouchableOpacity
                ...
                activeOpacity={0.75}
              >
                <View style={[styles.videoThumb, { backgroundColor: isDark ? COLORS.darkBorder : COLORS.bg3 }]}>
                ...
                <TouchableOpacity
                  onPress={() => handleOptionsPressVideo(video)}
                  style={{ padding: 8 }}
                  hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                >
        ...
        {currentFolders.length === 0 && matchingVideos.length === 0 && (
          <View style={[styles.emptyState, { borderColor: border }]}>
            <Ionicons
              name={searchQuery.trim() ? 'search-outline' : 'folder-open-outline'}
              size={40}
              color={subText}
            />
            <Text style={[styles.emptyText, { color: subText }]}>
              {searchQuery.trim() ? 'No videos found' : 'This folder is empty'}
            </Text>
          </View>
        )}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: isDark ? COLORS.darkCard : COLORS.white, borderColor: border }]}>
            <View style={styles.modalHeader}>
              <Text style={[styles.modalTitle, { color: text }]}>{editingFolder ? 'Edit Folder' : 'New Subfolder'}</Text>
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontFamily: FONTS.serifBold, fontSize: 24, letterSpacing: 0, marginBottom: 2 },
  headerSub: { fontSize: 12, fontWeight: '500' },
  addButton: { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center' },

  searchRow: { flexDirection: 'row', paddingHorizontal: 20, paddingTop: 14, marginBottom: 14, gap: 10 },
  searchContainer: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  sortButton: { width: 46, height: 46, borderRadius: 14, borderWidth: 1, justifyContent: 'center', alignItems: 'center' },

  content: { paddingHorizontal: 20, paddingTop: 8 },
  section: { marginBottom: 28 },
  sectionTitle: { fontSize: 13, fontWeight: '700', marginBottom: 12, letterSpacing: FONTS.tracking.wide, textTransform: 'uppercase' },
  ...
  folderIcon: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
  folderName: { fontSize: 14, fontWeight: '700', marginBottom: 3, letterSpacing: -0.2 },
  ...
  videoThumb: { width: 50, height: 50, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  videoInfo: { flex: 1 },
  videoTitle: { fontSize: 14, fontWeight: '700', marginBottom: 3, letterSpacing: -0.2 },
  ...
  modalInput: { borderWidth: 1, borderRadius: 14, paddingHorizontal: 16, height: 52, fontSize: 15, marginBottom: 24 },
