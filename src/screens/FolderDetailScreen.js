...
export default function FolderDetailScreen({ route, navigation }) {
  ...
  // Filter current folder's contents
  ...
  else currentFolders.sort((a,b) => b.id.localeCompare(a.id)); // Newest first based on ID timestamp

  const currentVideos = videos.filter(v => v.folder_id === folderId);
  const matchingVideos = searchQuery.trim() ? videos.filter(v => v.folder_id === folderId && v.title.toLowerCase().includes(searchQuery.toLowerCase())) : currentVideos;
      ...
      <View style={styles.background}>
        ...
        <View style={styles.headerContainer}>
          ...
          <View style={styles.headerTextContainer}>
            <Text style={[styles.headerTitle, { color: isDark ? '#FFB74D' : '#F5A623' }]}>{folderName}</Text>
            <Text style={[styles.folderCount, { color: subText }]}>{currentFolders.length} folders, {currentVideos.length} videos</Text>
          </View>
        ...
        >
                ...
                {currentFolders.map((folder) => (
                  ...
                  >
                    <View style={styles.folderCardHeader}>
                      ...
                      <TouchableOpacity onPress={() => handleOptionsPressFolder(folder)} style={styles.optionsBtn} hitSlop={{top: 10, bottom: 10, left: 10, right: 10}}>
          ...
          {matchingVideos.length > 0 && (
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: text }]}>{searchQuery.trim() ? "Video Results" : "Videos"}</Text>
              {matchingVideos.map((video) => (
                <TouchableOpacity 
                  key={video.id} 
                  style={[styles.videoCard, { backgroundColor: cardBg, borderColor: border, borderWidth: isDark ? 1 : 0 }]} 
                  activeOpacity={0.7}
                >
                  ...
                  <TouchableOpacity onPress={() => handleOptionsPressVideo(video)} style={{padding: 8}} hitSlop={{top: 10, bottom: 10, left: 10, right: 10}}>
          ...
          {currentFolders.length === 0 && matchingVideos.length === 0 && (
            <View style={styles.emptyState}>
              <Ionicons name={searchQuery.trim() ? "search-outline" : "folder-open-outline"} size={64} color={isDark ? border : COLORS.offWhite} />
              <Text style={[styles.emptyText, { color: subText }]}>{searchQuery.trim() ? "No videos found" : "This folder is empty"}</Text>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  background: { flex: 1 },
  headerContainer: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 24, paddingTop: 60, paddingBottom: 16 },
  backButton: { marginRight: 16 },
  headerTextContainer: { flex: 1 },
  headerTitle: { fontSize: 28, fontWeight: '800', color: '#F5A623', marginBottom: 2 }, // Orange
  ...
  emptyText: { marginTop: 16, fontSize: 16, color: COLORS.gray, fontWeight: '500' },
